  ...
  const populateApproverFields = () => {
    if (data.length > 0) {
      ...
      const approverFields = approvers.flatMap((approver, idx) => [
        { section: `Approver ${idx + 1}`, fields: [
          { fieldName: "Approver", field_type: "text", value: approver.approver, isMandatory: false },
          { fieldName: "Date of Approval", field_type: "date", value: "", isMandatory: false },
          { fieldName: "Approver Comment", field_type: "text", value: "", isMandatory: true },
        ]}
      ]);
  ...
  const populateReviewerFields = () => {
    if (data.length > 0) {
      ...
      const reviewerFields = reviewers.flatMap((reviewer, idx) => [
        { section: `Reviewer ${idx + 1}`, fields: [
          { fieldName: "Reviewer", field_type: "text", value: reviewer.reviewer, isMandatory: false },
          { fieldName: "Date of Review", field_type: "date", value: "", isMandatory: false },
          { fieldName: "Reviewer Comment", field_type: "text", value: "", isMandatory: true },
        ]}
      ]);
    ...
    {activeField ? (
      ...
      <div className="relative h-screen">
        <div className="overflow-auto mb-16">
          {activeDefaultTab === "Initiator Remarks" && fields["Initiator Remarks"]?.length > 0 && (
            <div>
              <div className="grid grid-cols-2 gap-4">
                {fields["Initiator Remarks"].map((field, index) => (
                  <div key={index} className="p-4 flex flex-col bg-white shadow border border-gray-300 mb-4">
          ...
          {activeDefaultTab !== "Initiator Remarks" && fields[activeDefaultTab]?.map((section, secIndex) => (
            <div key={secIndex} className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {section.fields?.map((field, index) => (
                  <div key={index} className="p-4 flex flex-col bg-white shadow border border-gray-300">
                    <label className="text-lg font-extrabold text-gray-700 mb-2">
                      {field.fieldName}
                      {field.isMandatory && <span className="text-red-500"> *</span>}
                    </label>
        ...
        <div className="fixed bottom-0 left-0 w-full bg-white  border-gray-300 p-4 flex justify-end gap-5">
          <AtmButton label={"Send For Review"} className='bg-blue-500 hover:bg-blue-700 p-2' />
          <AtmButton label={"Send For Approval"} className='bg-blue-500 hover:bg-blue-700 p-2' />
          <AtmButton label={"Open BMR"} className='bg-blue-500 hover:bg-blue-700 p-2' />
          <AtmButton label={"Exit"} className='bg-blue-500 hover:bg-blue-700 p-2' />
        </div>
      </div>
    )}
