...
const AddTabModal = ({ closeModal, addTab, updateTab, bmr_tab_id, existingTabName }) => {
  // Set tabName directly from props if in edit mode
  const [tabName, setTabName] = useState(updateTab === "edit" ? existingTabName : "");
  ...
  const handleSave = async () => {
    if (updateTab === "add") {
      try {
        const response = await axios.post("http://195.35.6.197:7000/bmr-form/add-bmr-tab", {
          bmr_id: bmr_id,
          tab_name: tabName
        },
        {
      ...
      try {
        const response = await axios.put(`http://195.35.6.197:7000/bmr-form/edit-bmr-tab/${bmr_tab_id}`, {
          bmr_id: bmr_id,
          tab_name: tabName
        },
        {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-opacity-50 backdrop-filter backdrop-blur-sm">
      <div className="bg-white p-4 rounded shadow-lg" style={{ width: '400px' }}>
        <h2 className="text-lg font-bold mb-4">{updateTab === "add" ? "Add Tab" : "Edit Tab"}</h2>
        <input
          type="text"
