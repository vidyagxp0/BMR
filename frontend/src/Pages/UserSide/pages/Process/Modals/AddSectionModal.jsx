...
const AddSectionModal = ({ closeModal, addSection,bmr_tab_id , bmr_section_id , updateSection , existingSectionName }) => {
  ...
  if(updateSection === "add-section"){
    try {
      const response = await axios.post(`http://192.168.1.16:7000/bmr-form/add-bmr-section`,
        {
          bmr_id: bmr_id,
      ...
      try {
        const response = await axios.put(`http://192.168.1.16:7000/bmr-form/edit-bmr-section/${bmr_section_id}`,
          {
            bmr_id: bmr_id,
            bmr_tab_id: bmr_tab_id,
            section_name: sectionName,
      ...
      <div>
        <div className="fixed inset-0 flex items-center justify-center bg-opacity-50 backdrop-filter backdrop-blur-sm">
          <div className="bg-white p-4 rounded shadow-lg" style={{ width: '400px' }}>
            <h2 className="text-lg font-bold mb-4">{updateSection === "add-section" ? "Add Section" : "Edit Section"}</h2>
