...
const AddSectionModal = ({ closeModal, addSection,bmr_tab_id , bmr_section_id , updateSection , existingSectionName }) => {
  ...
  if(updateSection === "add-section"){
    try {
      const response = await axios.post(`http://localhost:7000/bmr-form/add-bmr-section`,
        {
          bmr_id: bmr_id,
      ...
      try {
        const response = await axios.put(`http://localhost:7000/bmr-form/edit-bmr-section/${bmr_section_id}`,
          {
            bmr_id: bmr_id,
            bmr_tab_id: bmr_tab_id,
            section_name: sectionName,
