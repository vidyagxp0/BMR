...
const DeleteModal = ({ onClose, id, newTab, setNewTab, newSection, section_id, itemType , fetchBMRData,bmr_field_id}) => {
    const handleDelete = async () => {
        if (itemType === "tab") {
            try {
                const response = await axios.delete(`http://localhost:7000/bmr-form/delete-bmr-tab/${id}`,
                    {
                        headers: {
            ...
            try {
                const response = await axios.delete(`http://localhost:7000/bmr-form/delete-bmr-section/${section_id}`,
                    {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem("user-token")}`,
                            "Content-Type": "application/json",
            ...
            try {
                const response = await axios.delete(`http://localhost:7000/bmr-form/delete-bmr-field/${bmr_field_id}`,
                    {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem("user-token")}`,
                            "Content-Type": "application/json",
